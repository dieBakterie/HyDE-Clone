...
if [ "${enableWallDcol}" -eq 0 ] && [[ "${reload_flag}" -eq 1 ]]; then
    ...
    echo ":: deploying ${lycrTheme} colors :: ${dcol_mode} wallpaper detected"
    mapfile -d '' -t deployList < <(find "${lycrThemeDir}" -type f -name "*.theme" -print0)

    while read -r pKey; do
        fKey="$(find "${lycrThemeDir}" -type f -name "$(basename "${pKey%.dcol}.theme")")"
...
if [ "${enableWallDcol}" -eq 0 ]; then
    colorScheme="$({ grep -q "^[[:space:]]*\$COLOR-SCHEME\s*=" "${lycrThemeDir}/hypr.theme" && grep "^[[:space:]]*\$COLOR-SCHEME\s*=" "${lycrThemeDir}/hypr.theme" | cut -d '=' -f2 | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'; } || 
                    grep 'gsettings set org.gnome.desktop.interface color-scheme' "${lycrThemeDir}/hypr.theme" | awk -F "'" '{print $((NF - 1))}')"    colorScheme=${colorScheme:-$(gsettings get org.gnome.desktop.interface color-scheme)}
